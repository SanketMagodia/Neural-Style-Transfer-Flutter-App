...
def endGAME(content_path, style_path,iter=10):
  best, best_loss = run_style_transfer(content_path, style_path, num_iterations=iter)
  Image.fromarray(best)
  buffered = BytesIO()
  Image.fromarray(best).save(buffered, format="JPEG")
  img_str = base64.b64encode(buffered.getvalue())
  return img_str
@app.route('/', methods=['POST'])
def test():
    #r = request
    req1=request.form['image1']
    req2=request.form['image2']
    req3=int(request.form['iter'])
    # convert string of image data to uint8
    # nparr = np.fromstring(r.data['image'], np.uint8)
  

   
    imgdata1 = base64.b64decode(req1)
    filename = 'some_image1.jpg'  
    with open(filename, 'wb') as f:
        f.write(imgdata1)
    
    imgdata2 = base64.b64decode(req2)
    ...
    response_pickled = jsonpickle.encode(str(response))
